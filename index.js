...
app.get("/states/highest-gdp",(req,res)=>{
    const highestGdp = states.reduce((max,current)=>(current.gdp>max.gdp?current:max))
...
app.get("/states/:id",(req,res)=>{
    ...
    const filter = states.find((p)=>(p.id===id));
...
app.post("/states",(req,res)=>{
    ...
    const newId = states.length>0?Math.max(...states.map((p)=>(p.id)))+1:1;
...
app.delete("/states/low-literacy/:percentage", (req, res) => {
    const pr = parseFloat(req.params.percentage);
...
app.listen(3000,()=>{console.log("Server statrted")});